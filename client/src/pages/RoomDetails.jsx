  ...
  return room && (
    <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32'>
        ...
        <div className='flex flex-col md:flex-row items-start md:items-center gap-2'>
            ...
            <p className='text-xs font-inter py-1.5 px-3 text-white bg-red-500 rounded-full'>
                20% OFF
            </p>
        </div>
        {/*Room Rating */}
        ...
        <div className='flex flex-col lg:flex-row gap-6 mt-6'>
            <div className='lg:w-1/2 w-full overflow-hidden rounded-xl'>
                <img
                    src={mainImage}
                    alt="room image"
                    className='w-full shadow-lg object-cover transform transition-transform duration-300 ease-in-out hover:scale-105 hover:shadow-2xl cursor-pointer'
                />
            </div>
            <div className='grid grid-cols-2 gap-4 lg:w-1/2 w-full'>
                {room?.images?.length > 1 && room.images.map((image, index) => (
                    <img
                        ...
                        src={image}
                        alt={`room-${index}`}
                        className={`w-full rounded-xl shadow-md object-cover cursor-pointer ${mainImage === image ? 'ring-4 ring-orange-300' : ''}`}
                    />
                ))}
            </div>
        </div>
        ...
        <form className='flex flex-col md:flex-row items-start md:items-center justify-between bg-white shadow-[0px_0px_20px_rgba(0,0,0,0.15)] p-6 rounded-xl mx-auto mt-16 max-w-6xl'>
            <div className='flex flex-col flex-wrap md:flex-row items-start md:items-center gap-4 md:gap-10 text-gray-500'>
                ...
                <div className='flex flex-col'>
                    ...
                    <input type='number' id='guests' placeholder='Number of Guests' min={1} defaultValue={1} className='max-w-[80px] rounded border border-gray-300 px-3 py-2 mt-1.5 outline-none' required/>
                </div>
            </div>
            <button type='submit' className='bg-primary hover:bg-primary-dull active:scale-95 transition-all text-white rounded-md max-md:w-full max-md:mt-6 px-6 py-3 md:py-4 text-base cursor-pointer'>
                Check Availability
            </button>
        </form>
        ...
        <div className='flex flex-col items-start gap-4'>
            <div className='flex gap-4'>
                <img
                    src={'/host.jpg'}
                    alt={`Host - ${room.hotel?.owner?.username || room.hotel.name}`}
